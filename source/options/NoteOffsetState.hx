...
class NoteOffsetState extends MusicBeatState
{
	...
	var dumbMap:Map<String, Bool> = new Map<String, Bool>();
					...
					startMousePos.y - comboSpr.y >= 0 && startMousePos.y - comboSpr.y <= comboSpr.height)
		   {
			...
			if(dumbMap != null)
			{
				if(FlxG.mouse.justMoved)
				{
					...
					var addNum:Int = -2;
					if (dumbMap.get('rating') && dumbMap.exists('rating'))
						addNum = 0;
					else if (dumbMap.get('nums') && dumbMap.exists('nums'))
						addNum = 2;
					else if (dumbMap.get('combo') && dumbMap.exists('combo'))
						addNum = 4;
					if (addNum >= 0){
