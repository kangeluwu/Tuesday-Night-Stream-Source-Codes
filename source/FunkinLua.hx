...
class FunkinLua {
	...
	public function new(script:String) {
		...
		set('ratingName', '');
		set('ratingFC', '');
		set('version', MainMenuState.RCEVersion.trim());
		...
		set('botPlay', PlayState.instance.botplay);
		...
		set('comboOffsetsX', ClientPrefs.comboOffset[0]);
		set('comboOffsetsY', ClientPrefs.comboOffset[1]);
		#if windows
		...
		/*Lua_helper.add_callback(lua, "setSettingsBool", function(boolName:Dyamic = null,isreal:Bool){
			if(boolName==null){
				#if (linc_luajit >= "0.0.6")
				LuaL.error(lua, "the Bool is NULL!WTH ARE YOU DOING???");
				#end
				return;
			}
			ClientPrefs.boolName = isreal;
		});*/
//DUMB SETTING LOLOLLOLOLOLOLOLOLLLLLLOLL
		...
		Lua_helper.add_callback(lua, "removeCharacterFromList", function(name:String, type:String) {
			var charType:Int = 0;
			switch(type.toLowerCase()) {
				case 'dad': charType = 1;
				case 'gf' | 'girlfriend': charType = 2;
			}
			PlayState.instance.removeCharacterFromList(name, charType);
		});
		...
		Lua_helper.add_callback(lua, "exitSong", function(?skipTransition:Bool = false) {
			...
			CustomFadeTransition.nextCamera = PlayState.instance.camHUD2;
		...
		Lua_helper.add_callback(lua, "characterDance", function(character:String) {
			switch(character.toLowerCase()) {
				...
				case 'bf' | 'boyfriend': PlayState.instance.boyfriend.dance();
				default: if(PlayState.instance.getLuaCharacter(character)!=null) {
					var char:ModchartCharacter = PlayState.instance.getLuaCharacter(character);
					char.dance();
			}
			else
				{
		...
		Lua_helper.add_callback(lua, "addLuaCharacter", function(tag:String, charName:String,front:Bool = false) {
		...
		Lua_helper.add_callback(lua, "makeLuaCharacter", function(tag:String, charName:String, x:Float, y:Float,flipXLol:Bool) {
			tag = tag.replace('.', '');
			resetCharacterTag(tag);
			var leChar:ModchartCharacter = new ModchartCharacter(x, y, charName, flipXLol);

			leChar.antialiasing = ClientPrefs.globalAntialiasing;
			PlayState.instance.modchartCharacters.set(tag, leChar);
			leChar.active = true;
		});
		...
		Lua_helper.add_callback(lua, "getTextFromFile", function(path:String, ?ignoreModFolders:Bool = false, ?library:Null<String> = null) {
			return Paths.getTextFromFile(path, library,ignoreModFolders);
		});
		...
		Lua_helper.add_callback(lua, "setLuaCharacterCamera", function(tag:String, camera:String = '') {
			
			if(PlayState.instance.modchartCharacters.exists(tag)) {
				PlayState.instance.modchartCharacters.get(tag).cameras = [cameraFromString(camera)];
				return true;
			}
			luaTrace("Lua character with tag: " + tag + " doesn't exist!");
			return false;
		});
		Lua_helper.add_callback(lua, "setLuaCharacterScrollFactor", function(tag:String, scrollX:Float, scrollY:Float) {
			
			if(PlayState.instance.modchartCharacters.exists(tag)) {
				PlayState.instance.modchartCharacters.get(tag).scrollFactor.set(scrollX, scrollY);
				return true;
			}
			return false;
		});
		Lua_helper.add_callback(lua, "scaleLuaCharacter", function(tag:String, x:Float, y:Float) {
			if(PlayState.instance.modchartCharacters.exists(tag)) {
				var shit:ModchartCharacter = PlayState.instance.modchartCharacters.get(tag);
				shit.scale.set(x, y);
				shit.updateHitbox();
	...
	public function initHaxeInterp()
	{
		...
		haxeInterp.variables.set("ChartingState", editors.ChartingState);
		#if sys
	...
	function resetCharacterTag(tag:String) {
		...
		var char:ModchartCharacter = PlayState.instance.modchartCharacters.get(tag);
		char.kill();
		if(char.wasAdded) {
			PlayState.instance.remove(char, true);
		}
		char.destroy();
	...
	function cameraFromString(cam:String):FlxCamera {
		switch(cam.toLowerCase()) {
			...
			case 'camHUD2' | 'other': return PlayState.instance.camHUD2;
		}
...
class ModchartCharacter extends Character
{
	...
	public function new(?x:Float = 0, ?y:Float = 0, ?character:String = 'bf', ?flipX:Bool = false)
	{
		super(x, y, character, flipX);
		antialiasing = ClientPrefs.globalAntialiasing;
	}
}
