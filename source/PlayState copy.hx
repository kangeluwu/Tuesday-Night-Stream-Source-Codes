...
class PlayState extends MusicBeatState
{
	...
	public var camHUD2Shaders:Array<ShaderEffect> = [];
	
	public var strumLineNotes:FlxTypedGroup<StrumNote>;
	public var opponentStrums:FlxTypedGroup<StrumNote>;
	public var playerStrums:FlxTypedGroup<StrumNote>;
	public var grpNoteSplashes:FlxTypedGroup<NoteSplash>;
	public var grpCrossfades:FlxTypedGroup<CrossFade>;
	public var grpGFCrossfades:FlxTypedGroup<CrossFade>;
	public var grpBFCrossfades:FlxTypedGroup<BFCrossFade>;
	...
	public var funniCrossFades:Array<CrossFade> = [];
	...
	public function makeHaxeState(usehaxe:String, path:String, filename:String) {
		...
		interp.variables.set("BFCrossFade", BFCrossFade);
...
//ClientPrefs:啊对对对我是颜色是吧
	...
	{
		var modsJson = CoolUtil.parseJson(FNFAssets.getText("mods/stages/custom_Hscript_stages/custom_stages.json"));
		var stageJson = CoolUtil.parseJson(FNFAssets.getText("corrupted_assets/stages/custom_Hscript_stages/custom_stages.json"));
		...
		grpGFCrossfades = new FlxTypedGroup<CrossFade>();
		grpCrossfades = new FlxTypedGroup<CrossFade>();
		grpBFCrossfades = new FlxTypedGroup<BFCrossFade>();

		if (FNFAssets.exists((Paths.getHscriptStagePath(Reflect.field(stageJson, curStage))), Hscript))
			{

		makeHaxeState("stages", "corrupted_assets/stages/custom_Hscript_stages/" + curStage + "/", "../"+Reflect.field(stageJson, curStage));
			}//HSCRIPT GOD YO
			else if (FNFAssets.exists((Paths.modsHscriptStages(Reflect.field(modsJson, curStage))), Hscript))
				{

			makeHaxeState("stages", "mods/stages/custom_Hscript_stages/" + curStage + "/", "../"+Reflect.field(modsJson, curStage));
				}//HSCRIPT GOD YO
				else
			{
	...
	if (isFixedAspectRatio)
		...
		if (isCorruptUI){
			...
			if(!isPixelStage){
				introSoundsSuffix = '-corrupted';
				}else if(isPixelStage) {
					introSoundsSuffix = '-corruptedPixel';
				}
		}
        /*else if (usesCustom != ''){
		...
		if (FNFAssets.exists('corrupted_assets/data/' + SONG.song.toLowerCase() + '/dialog.txt'))
			{
				filename = 'corrupted_assets/data/' + SONG.song.toLowerCase() + '/dialog.txt';
			}
		...
		if (customTimeBar == '')
			...
			timeBarBG = new AttachedSprite(customTimeBar, null,  null,  null, timeBarPath);
		...
		if (FNFAssets.exists("corrupted_assets/custom_events/events", Hscript))
			{
				makeHaxeState("events", "corrupted_assets/custom_events/", "events");
			}
		...
		moveCameraSection();
		if (customHealthBar == '' && !isCorruptUI)
			healthBarBG = new AttachedSprite('healthBar');
		else if (isCorruptUI)
			healthBarBG = new AttachedSprite('healthBarc');
			else
			healthBarBG = new AttachedSprite(customHealthBar, null,  null,  null, healthBarPath);
	...
	function customIntro(?dialogueBox:DialogueBoxMPlus) {
		var goodJson = CoolUtil.parseJson(FNFAssets.getText('corrupted_assets/scripts/custom_cutscenes/cutscenes.json'));
		var modJson = CoolUtil.parseJson(FNFAssets.getText('mods/scripts/custom_cutscenes/cutscenes.json'));
		if (!Reflect.hasField(goodJson, SONG.cutsceneType) && !Reflect.hasField(modJson, SONG.cutsceneType)) {
		...
		if (Reflect.hasField(goodJson, SONG.cutsceneType))
			{
				makeHaxeState("cutscene", "corrupted_assets/scripts/custom_cutscenes/"+SONG.cutsceneType+'/', "../"+Reflect.field(goodJson, SONG.cutsceneType));
			} 
		if (Reflect.hasField(modJson, SONG.cutsceneType))
			{
				makeHaxeState("cutscene", "mods/scripts/custom_cutscenes/"+SONG.cutsceneType+'/', "../"+Reflect.field(modJson, SONG.cutsceneType));
			}
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			if (FNFAssets.exists("corrupted_assets/data/" + SONG.song.toLowerCase() + "/modchart", Hscript))
			{
				makeHaxeState("modchart", "corrupted_assets/data/" + SONG.song.toLowerCase() + "/", "modchart");
				
			}


			if (FNFAssets.exists("mods/data/" + SONG.song.toLowerCase() + "/modchart", Hscript))
			{
				makeHaxeState("modchart", "mods/data/" + SONG.song.toLowerCase() + "/", "modchart");
				
			}

			if (FNFAssets.exists("mods/" + Paths.currentModDirectory + "/data/" + SONG.song.toLowerCase() + "/modchart", Hscript))
				{
					makeHaxeState("modchart", "mods/" + Paths.currentModDirectory + "/data/" + SONG.song.toLowerCase() + "/", "modchart");
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				introAssets.set('corrupted', ['Corruption/ready', 'Corruption/set', 'Corruption/go']);
				introAssets.set('corruptedPixel', ['Corruption/pixelUI/ready-pixel', 'Corruption/pixelUI/set-pixel', 'Corruption/pixelUI/date-pixel']);
				...
				if(isCorruptUI) {
					...
					introAlts = introAssets.get('corrupted');
						...
						introAlts = introAssets.get('corruptedPixel');
						antialias = false;
					}
				}
	...
	public function updateScore(miss:Bool = false)
	{
		...
		+ " | " + "(" + ratingFC + ")" + accuracyShits(ratingPercent * 100)
		+ " | Rating : " + ratingName;
		}
		else
		{
...
function eventPushed(event:EventNote) {
	...
	{
		isGhost = ClientPrefs.ghostTapping;
	if (isFixedAspectRatio)
			FlxG.fullscreen = false;
		if (storyDifficulty == 0 && isStoryMode)
		ClientPrefs.ghostTapping = true;
		else if (storyDifficulty >= 0 && isStoryMode)
		ClientPrefs.ghostTapping = false;
		else ClientPrefs.ghostTapping = isGhost;
		...
		if ((!genocideMode && healthBar.percent > 80) || (genocideMode && (health / 2 * 100) > 100)) {
		...
		if (unspawnNotes[0] != null)
		{
			...
			while (unspawnNotes.length > 0 && unspawnNotes[0].strumTime - Conductor.songPosition < time)
			{
				...
				callAllHScript('onSpawnNote', [notes.members.indexOf(dunceNote), dunceNote.noteData, dunceNote.noteType, dunceNote.isSustainNote]);
...
public function addShaderToCamera(cam:String, effect:ShaderEffect)
	{ // STOLE FROM ANDROMEDA
		...
		switch (cam.toLowerCase())
		{
			...
			case 'camother' | 'other' | 'camhud2' | 'hud2':
				camHUD2Shaders.push(effect);
				var newCamEffects:Array<BitmapFilter> = []; // IT SHUTS HAXE UP IDK WHY BUT WHATEVER IDK WHY I CANT JUST ARRAY<SHADERFILTER>
				for (i in camHUD2Shaders)
				{
					newCamEffects.push(new ShaderFilter(i.shader));
				}
				camHUD2.setFilters(newCamEffects);
			case 'camgame' | 'game':
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String, value3:String) {
		switch(eventName) {
			...
			case 'Hey!':
				...
				if(value != 1 && !boyfriend.curCharacter.startsWith('gf')) {
...
FlxTween.tween(FlxG.camera, {zoom: zooms}, time, {ease: FlxEase.cubeInOut, onComplete:
			...
			case 'Screen Shake':
				...
				for (i in 0...targetsArray.length) {
					...
					if(split[2] != null) intensity = Std.parseFloat(split[2].trim());
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if(PlayState.isCorruptUI)
		{
			if (!PlayState.isPixelStage)
				{
					backShitPart1 = 'Corruption/';
				}
			else
				{
					backShitPart1 = 'Corruption/pixelUI/';
					backShitPart2 = '-pixel';
				}
		}
		rating.loadGraphic(Paths.image(backShitPart1 + daRating.image + backShitPart2));
		...
		var comboSpr:FlxSprite = new FlxSprite().loadGraphic(Paths.image(backShitPart1 + 'combo' + backShitPart2));
		...
		{
			var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image(backShitPart1 + 'num' + Std.int(i) + backShitPart2));
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if ((storyDifficulty == 2) && isStoryMode)
			health -= daNote.missHealth * healthLoss * 1.5;
			else 
	...
	var altNum:Int = 0;
	function oppSing(note:Note):Void
	{
			...
			if (!note.wasGoodHit)
			{
				...
				if(!note.noAnimation) {
					...
					if(note.noteType == 'Hey!') {
						if(boyfriend.animOffsets.exists('hey') && !boyfriend.curCharacter.startsWith('gf')) {
							boyfriend.playAnim('hey', true);
							boyfriend.specialAnim = true;
						...
						if(gf != null && gf.animOffsets.exists('cheer') ||boyfriend.curCharacter.startsWith('gf') && gf.animOffsets.exists('cheer')) {
							gf.playAnim('cheer', true);
							gf.specialAnim = true;
							gf.heyTimer = 0.6;
						}
					...
					if(note.noteType == 'Warning Note') {
						if(boyfriend.animOffsets.exists('dodge') && !boyfriend.curCharacter.startsWith('gf')) {
							boyfriend.playAnim('dodge', true);
							boyfriend.specialAnim = true;
						}
						if(gf.animOffsets.exists('dodge') && boyfriend.curCharacter.startsWith('gf')) {
				...
				var leData:Int = Math.round(Math.abs(note.noteData));
				var leType:String = note.noteType;
				callOnLuas('goodNosteHit', [notes.members.indexOf(note), leData, leType, isSus]);
...
public function makeCrossfades(/*toPlayer:Bool,char:Character = null ,isgf:Bool, isbf:Bool = null, bfChar:Boyfriend = null*/):Void
	{
	/*	if (isgf && (!isbf || isbf == null) && gf != null){//为什么GirlFriend同志总是如此特殊（恼）
			var gfcross:CrossFade = new CrossFade(char,grpGFCrossfades,toPlayer);
			} else 
			if (!isgf && (!isbf || isbf == null)){
		var cross:CrossFade = new CrossFade(char,grpCrossfades,toPlayer);
			} else 
			if (isbf && isbf != null && !isgf){
		var bFcross:CrossFade = new CrossFade(bfChar,grpBFCrossfades,toPlayer);

			}

			cross.duration = cfDuration;
			cross.alpha = cfIntensity;
			cross.blend = blendModeFromString(cfBlend);
		*/	}
	...
	function staticHitMiss(){
		trace('lol you missed the static note!');
					daNoteStatic = new FlxSprite(0, 0).loadGraphic(Paths.image("hitStatic"));
