...
var instStuff:String = '';
var voicesStuff:String = '';
	...
	function addNoteUI():Void
	{
		...
		for (i in 0...directories.length) {
			...
			if(FileSystem.exists(directory)) {
				for (file in FileSystem.readDirectory(directory)) {
					var path = haxe.io.Path.join([directory, file]);
					if (!FileSystem.isDirectory(path) && (file.endsWith('.lua') || file.endsWith('.hscript') )) {
						var fileToCheck:String = file.substr(0, file.endsWith('.hscript') ? file.length - 8 : file.length - 4);
...
case 'Alt Anim Note':
	if (curSelectedNote != null) {
		curSelectedNote[4] = check.checked ? 1 : 0;
	}
	updateNoteUI();
			}
		}
	...
	function updateNoteUI():Void
	{
		if (curSelectedNote != null) {
			if(curSelectedNote[2] != null) {
				...
				isAltNoteCheck.checked = cast curSelectedNote[4];
			stepperAltNote.value = curSelectedNote[4] != null ? curSelectedNote[4] : 0;
...
for (i in _song.events)
{
	...
	function changeKeyType(change:Int) {
		noteType += change;
		if (noteType > noteTypeList.length)
			noteType = 0;
